...
async function microsoftTranslate(original, destlang, e, apikeyMicrosoft, preverbs, rowId, transtype, plural_line, locale, convertToLower) {
    ...
    let result = await getTrans(e, destlang, apikeyMicrosoft, original, originalPreProcessed, rowId, trntype, transtype, plural_line, locale, convertToLower);
    return errorstate;
}

async function getTrans(record, language, apikeyMicrosoft, original, originalPreProcessed, rowId, trntype, transtype, plural_line, locale, convertToLower) {
    var row = "";
    var translatedText = "";
    var ul = "";
    var current = "";
    ...
    const myRequest = new Request('flowers.jpg', {
       ...
       mode: 'cors',
       cache: 'default',
       body: JSON.stringify(originalPreProcessed)
    });
    ...
    let link = "https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&textType=" + trntype + "&from=en&to=" + language
        ...
        .then(async response => {
            ...
            if (!response.ok) {
                ...
                if (typeof data != "undefined") {
                    ...
                    data = "noData";
                    error = [data, message, response.error];
                }
