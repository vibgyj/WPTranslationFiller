...
function postProcessTranslation(original, translatedText, replaceVerb, originalPreProcessed, translator, convertToLower, spellCheckIgnore, locale, replLog, mycontext="") {
    ...
    // if we have reasoning models then we need to remove the <think> tags and the content between them, because that is not part of the translation but only for reasoning
   ...
   .replace(/<think>[\s\S]*?<\/think>\s*/gi, '')
    ...
    if (translator == "google") {
        ...
        //console.debug("we are in Ollama post process before replacing placeholders:", translatedText)
        const placeholderRegex = /%(\d{1,2})?\$?[sdl]{1}|&#\d{1,4};|&#x\d{1,4};|&\w{2,6};|%\w*%/gi;
        // We need to put back the %s etc
       ...
       // const hasTrailingNewline = /\r?\n$/.test(original);
        ...
        if (markupmatches != null) {
          ...
          const ollamamatches = original.matchAll(placeHolderRegex);
        if (ollamamatches != null) {
            let index = 0;
            for (const match of ollamamatches) {
        ...
        if (markupmatches != null) {
            for (const markupmatch of markupmatches) {
                translatedText = translatedText.replace(`{myymark_var${index}}`, markupmatch);
                index++;
            }
        }
  ...
  if (toBoolean(DebugMode)) console.debug("After replacement 830:", translatedText);
    ...
    if (codematches != null) {
       let index = 1;
       for (const match of codematches) {
            translatedText = translatedText.replace(`codevar${index}`, match);
            index++;
         }
    }
...
function replace_special_var(original, translatedNewText) {
    //console.log('RAW:', JSON.stringify(translatedNewText));
    //const specialChar = /<[^>]*>|&#[0-9]+;|&[a-z]+;|\r\n|\r|\n|\t|[#?]/g;
    const specialChar = /<[^>]*>|&#[0-9]+;|&[a-z]+;|\r\n|\r|\n|\t|#|[\u2013\u2014\u2018\u2019\u201C\u201D\u2026]/g;
        ...
        for (let e of document.querySelectorAll("tr.editor div.editor-panel__left div.panel-content")) {
            ...
            if (toTranslate) {
                ...
                if (transtype == "single") {
                    ...
                    repl_array = repl_array.concat(result.repl_array);
                    for (const p of result.repl_array) allReplacements.push([p[0], p[1], countrows]);
                    ...
                    if (replaced) {
                        ...
                        if (preview != null) {
                            ...
                            preview.innerText = previewNewText;
                        }
                        else {
                            preview = document.querySelector("#preview-" + myrow + " td.translation");
                        }
                        // 16-04-2023 fix for issue #293 marking of replaced words did not work anymore
                        markElements(preview, replaceVerb, orgText, spellcheckIgnore, repl_array,previewNewText,newrowId);
                    }
                }
                else {
                    // plural line 1
                    ...
                    if (replaced) {
                        ...
                        // 16-04-2023 fix for issue #293 marking of replaced words did not work anymore
                        markElements(previewElem, replaceVerb, orgText, spellcheckIgnore, repl_verb, previewNewText,newrowId);
                    }
                    // plural line 2
                    ...
                    if (previewElem != null) {
                        ...
                        if (replaced) {
                            ...
                            if (textareaElem1 != null) {
                                ...
                                textareaElem1.value = translatedText;
                                // 16-04-2023 fix for issue #293 marking of replaced words did not work anymore
                                markElements(previewElem, replaceVerb, orgText, spellcheckIgnore, repl_verb, previewNewText,rowId);
                            }
                        }
                    }
                }
...
async function checkPage(postTranslationReplace, formal, destlang, apikeyOpenAI, OpenAIPrompt, spellcheckIgnore, showHistory, openAIkey, OpenAIPrompt, reviewPrompt) {
    ...
    try {
    const myheader = document.querySelector('header, .wp-block-group.global-header');
    //console.debug("myheader:", myheader);

    progressbar = document.querySelector(".indeterminate-progress-bar");
  //  console.debug("progressbar:", progressbar, "connected:", progressbar?.isConnected, "parent:", progressbar?.parentElement);

    if (progressbar == null) {
        //console.debug("progressbar is null, creating new one");
         progressbar = createProgressBar();
        if (myheader != null) {
          myheader.after(progressbar);
         }
   ...
   // console.debug("after show — display:", getComputedStyle(progressbar).display, "height:", getComputedStyle(progressbar).height);
    if (is_pte) {
        ...
        if (postTranslationReplace.length != 0 && postTranslationReplace != "undefined") {
            ...
            for (const record of records) {
                ...
                //console.debug("row:", row, newrowId)
                if (mypreview == null) {
                ...
                if (!mypreview.classList.contains('no-translations')) {
                    // 30-08-2021 PSS fix for issue # 125
                    let precomment = record.querySelector(".source-details__comment p");
                    if (precomment != null) {
                        comment = precomment.innerText;
                        comment = comment.replace(/(\r\n|\n|\r)/gm, "");
                        toTranslate = checkComments(comment.trim());
                    ...
                    if (toTranslate == true) {
                        ...
                        if (transtype == "single") {
                            // Fetch the translations
                            //console.debug("single translation found for row:", row)
                            ...
                            if (translatedText != "No suggestions" && translatedText != "") {
                                ...
                                // --- 1. ALL replacements first, no DOM writes ---
                                result = check_start_end(translatedText, previewNewText, recWordCount, repl_verb, original, replaced, countrows);
                                ...
                                result = replElements(translatedText, translatedText, replaceVerb, repl_verb, "", original, countrows, spellcheckIgnore);
                                repl_array = repl_array.concat(result.repl_array);
                                for (const p of result.repl_array) allReplacements.push([p[0], p[1], countrows]);
                                //console.debug("count +=", typeof result.countreplaced, result.countreplaced);
                                previewNewText = result.previewNewText;
                                translatedText = result.translatedText;
                                countreplaced + result.countreplaced;
                                orgText = result.orgText;
                                if (result.replaced) {
                                    replaced = true;
                                    repl_verb += result.repl_verb;
                                    repl_array = repl_array.concat(result.repl_array);   // concat, not +=
                                    recWordCount += result.countreplaced;
                                }

                                // --- 2. one DOM write, one updateStyle, then mark LAST ---
                                if (toBoolean(replaced)) {
                                    // fresh lookup for THIS row
                                    preview = document.querySelector("#preview-" + newrowId + " td.translation")
                                        || document.querySelector("#preview-" + row + " td.translation");
                                    ...
                                    if (preview != null) {
                                        let span = preview.querySelector("span.translation-text");
                                        if (span != null) span.remove();
                                        const myspan1 = document.createElement("span");
                                        myspan1.className = "translation-text";
                                        myspan1.textContent = previewNewText;
                                        preview.replaceChildren(myspan1);          // single, clean write
                                    }

                                    if (currec != null) {
                                        const current = currec.querySelector("span.panel-header__bubble");
                                        if (current) current.innerText = "transFill";
                                    }

                                    result = { wordCount: recWordCount, percent: 10, toolTip: "" };
                                    old_status = document.querySelector("#preview-" + row);
                                    updateStyle(textareaElem, result, "", 'True', false, false, row, record, showHistory, true, translatedText, repl_array, prev_trans, old_status, false);

                                    // marking happens LAST — nothing rewrites the cell after this
                                    if (preview != null) {
                                        markElements(preview, repl_array, original, spellcheckIgnore, repl_array, translatedText,row);
                                    }
                                }
                            }
                        }
                            ...
                            if (translatedText != "No suggestions") {
                                ...
                                if (replaced) {
                                    ...
                                    // markElements(previewElem1, repl_array, orgText, spellcheckIgnore, repl_array, translatedText);
                                ...
                                result = replElements(translatedText, previewNewText, replaceVerb, repl_verb, "", original, countrows, spellcheckIgnore);
                                  repl_array = repl_array.concat(result.repl_array);
                                for (const p of result.repl_array) allReplacements.push([p[0], p[1], countrows]);
                                //console.debug("result 2313:", result)
                                replaced = result.replaced;
                                orgText = result.orgText;
                                if (toBoolean(replaced)) {
                                    mypreview.classList.replace("status-current", "status-waiting");
                                    ...
                                    // Highlight all keywords found in the page, so loop through the replacement array
                                    // 16-04-2023 fix for issue #293 marking of replaced words did not work anymore
                                    markElements(previewElem1, repl_array, orgText, spellcheckIgnore, repl_array, translatedText, newrowId);
                                }
                            ...
                            if (previewElem2 != null) {
                                ...
                                if (translatedText != "No suggestions") {
                                    ...
                                    repl_array = result.repl_array;
                                    for (const p of result.repl_array) allReplacements.push([p[0], p[1], countrows]);
                                    //console.debug("result 2345:", result)
                                    replaced = result.replaced;
                                    repl_array += result.repl_array;
                                    if (replaced) {
                                        mypreview.classList.replace("status-current", "status-waiting");
                                        ...
                                        //markElements(previewElem, repl_array, orgText, spellcheckIgnore, repl_array, translatedText);
                                        // 09-09-2022 PSS fix for issue #244
                                        if (currec != null) {
                                            var current = currec.querySelector("span.panel-header__bubble");
                                            var prevstate = current.innerText;
                                    ...
                                    if (replaced) {
                                        ...
                                        // Highlight all keywords found in the page, so loop through the replacement array
                                        // 16-04-2023 fix for issue #293 marking of replaced words did not work anymore
                                        markElements(previewElem2, repl_array, orgText, spellcheckIgnore, repl_array, translatedText, newrowId);
                                    }
                                }
                            }
                            else {
                                console.debug("We do not have a second text within the plural")
                            }
                        }
                        if (replaced) {
                            // Only update the style if verbs are replaced!!
                            ...
                            let showName = false
                            updateStyle(textareaElem, result, "", 'True', showName, nameDiff, row, record, showHistory, true, translatedText, repl_array, prev_trans, old_status, false);
                        }
                    }
            ...
            //console.debug("we are done with all")
            // ===== completion — always runs, exactly once =====
            let repldone = __("Replace verbs done: ");
            let repltext = __(" replaced words<br>");
            //console.debug("repl_verb:", countreplaced)
            // after the loop:
            const replReport = allReplacements
            .map(([from, to, row]) => `${row} : ${from}->${to}`)
              .join("<br>");
           
            messageBox("info", repldone + recWordCount + repltext + "<br>" + replReport);
            const bar = document.querySelector(".indeterminate-progress-bar");
            if (bar) bar.style.display = "none";
            
            const doneButton = document.querySelector(".wptfNavBarCont a.check_translation-button");
            doneButton.innerText = "Checked";
            doneButton.className = "check_translation-button ready";
        ...
        console.error("checkPage DIED:", err);
        alert("checkPage died: " + err.message);   // impossible to miss
    }
...
async function markElements(preview, replaceVerb, orgText, spellcheckIgnore, repl_array, translatedText,new_row) {
    ...
    if (typeof repl_array != "undefined" && typeof nwText != 'undefined') {
        ...
        if (arr.length > 0) {
            ...
            if (toBoolean(DebugMode)) {
                console.debug("in document?", document.contains(preview),
                    "marks:", preview.querySelectorAll('.highlight').length);
                const mark_row = document.querySelector("#preview-" + new_row);   // adjust id
                console.debug("row:", mark_row)
                const orig = mark_row.querySelector("td.original");        // original cell
                const trans = mark_row.querySelector("td.translation");    // translation cell

                console.debug("ORIGINAL cell:");
                console.debug("  innerHTML:  ", orig?.innerHTML);
                console.debug("  innerText:  ", JSON.stringify(orig?.innerText));
                console.debug("  textContent:", JSON.stringify(orig?.textContent));

                console.debug("TRANSLATION cell:");
                console.debug("  innerHTML:  ", trans?.innerHTML);
                console.debug("  innerText:  ", JSON.stringify(trans?.innerText));
                console.debug("  textContent:", JSON.stringify(trans?.textContent));
            }
        }
    }
    else {
   ...
   // console.debug("markElements done, repl_array length:", repl_array.length)
}
...
function replElements(
    translatedText,
    previewNewText,
    replaceVerb,
    repl_verb,        // kept in signature for compatibility — IGNORED
    countreplaced,    // kept in signature for compatibility — IGNORED
    original,
    countrows,
    spellcheckIgnore
) {
    ...
    var repl_array = [];
    let myVerb = "";        // fresh delta per call
    let myCount = 0;        // fresh delta per call
    ...
    for (let i = 0; i < replaceVerb.length; i++) {
        ...
        if (translatedText.includes(searchWord)) {
            if (!CheckUrl(translatedText, searchWord)) {
                const wordRegex = new RegExp(escapeRegExp(searchWord), 'g');
                ...
                const replacer = (match, offset, fullString) => {
                    ...
                    const startsWithCapital =
                        match.charAt(0) !== match.charAt(0).toLowerCase();
                    return (isSentenceStart && startsWithCapital) ? match : replacement;
                };
                ...
                if (newTranslated !== translatedText) {
                    ...
                    myVerb += countrows + " : " + searchWord + "->" + replacement + "<br>";
                    repl_array.push([searchWord, replacement]);
                    myCount++;
                    replaced = true;
                }
    ...
    // --- Stap 2: koppeltekens check ---
    let lines = [];
    if (typeof spellcheckIgnore !== "undefined") {
        lines = spellcheckIgnore.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
    }

    translatedText.split(' ').forEach(word => {
        if (word === "->") return;
        let normalizedWord = word.replace(/[\u2010\u2011\u2012\u2013\u2014]/g, "-");
        let strippedWord = normalizedWord.replace(/[.:;!?\,]+$/g, "");
        let firstPart = strippedWord.split("-")[0];
        if (firstPart.startsWith("(")) firstPart = firstPart.substr(1);

        if (lines.some(ignore => firstPart === ignore)) {
            if (normalizedWord.includes("-") && normalizedWord !== "--" && normalizedWord !== "-") {
                let newWord = normalizedWord.replace(/-/g, " ");
                let hyphenCount = (normalizedWord.match(/-/g) || []).length;

                translatedText = translatedText.replaceAll(normalizedWord, newWord);
                previewNewText = previewNewText.replaceAll(normalizedWord, newWord);
                repl_array.push([normalizedWord, newWord]);
                myVerb += countrows + " : " + normalizedWord + "->" + newWord + "<br>";
                myCount += hyphenCount;
                replaced = true;
            }
        }
    ...
    return {
        ...
        previewNewText,
        translatedText,
        countreplaced: myCount,   // NUMBER, this call's delta only
        orgText,
        repl_verb: myVerb,        // this call's lines only
        repl_array
    };
}
    ...
    switch (type) {
        ...
        case 'single':
            ...
            // ── NEW: KoboldCPP plural ────────────────────────────────────────
                ...
                let editor = true;
                result = await NLPCloudTranslate(original, destlang, record, apikeyNLP, OpenAIPrompt, replacePreVerb, row, transtype, plural_line, formal, locale, convertToLower, editor, "1", MistralSelect, OpenAItemp, spellCheckIgnore, OpenAITone, "editor", openAiGloss);
                if (result == "Error 401") {
                    messageBox("error", __("Error in translation received status 401<br>The request is not authorized because credentials are missing or invalid."));
            ...
            else if (transsel == "Mistral") {
                let editor = true;
                result = await MistralTranslate(original, destlang, record, apikeyMistral, OpenAIPrompt, replacePreVerb, row, transtype, plural_line, formal, locale, convertToLower, editor, "1", MistralSelect, OpenAItemp, spellCheckIgnore, OpenAITone, "editor", openAiGloss);
                if (result == "Error 401") {
                    messageBox("error", __("Error in translation received status 401<br>The request is not authorized because credentials are missing or invalid."));
                    // alert("Error in translation received status 401 \r\nThe request is not authorized because credentials are missing or invalid.");
                }
    ...
    if (pretrans == "notFound") {
        if (transsel == "translation_io") {
          ...
          //  console.debug("mycontext in cerebras:", mycontext)
            ...
            let is_editor = true;
            result = await googleTranslate(plural, destlang, record, apikey, replacePreVerb, row, transtype, plural_line, locale, convertToLower, is_editor, spellCheckIgnore, false);
            if (errorstate == "Error 400") {
                messageBox("error", __("API key not valid. Please pass a valid API key.<br>Please check your licence in the options!!!"));
                //alert("API key not valid. Please pass a valid API key. \r\nPlease check your licence in the options!!!");
                // break;
            ...
            if (result == "Error 401") {
                ...
                if (errorstate != "OK") {
                    ...
                    //alert("There has been some uncatched error: " + errorstate);
                        ...
                        let editor = true;
                        result = await MistralTranslate(plural, destlang, record, apikeyMistral, OpenAIPrompt, replacePreVerb, row, transtype, plural_line, formal, locale, convertToLower, editor, "1", MistralSelect, OpenAItemp, spellCheckIgnore, OpenAITone, "editor", openAiGloss);
                        if (result == "Error 401") {
                            messageBox("error", __("Error in translation received status 401<br>The request is not authorized because credentials are missing or invalid."));
                            // alert("Error in translation received status 401 \r\nThe request is not authorized because credentials are missing or invalid.");
                        }
           ...
           result = await getTransOpenAI(plural, destlang, e, apikeyOpenAI, OpenAIPrompt, replacePreVerb, rowId, transtype, plural_line, locale, convertToLower, DeeplFree, editor, "1", OpenAISelect, OpenAItemp, spellCheckIgnore, OpenAITone, "editor", openAiGloss,URL,mycontext);
            ...
            if (result == "Error 401") {
                ...
                if (errorstate != "OK") {
                 ...
                 let editor = false;
                 result = await groqTranslate(plural, destlang, record, apikeygroq, OpenAIPrompt, replacePreVerb, row, transtype, plural_line, formal, locale, convertToLower, editor, "1", groqSelect, OpenAItemp, spellCheckIgnore, OpenAITone, "editor", openAiGloss);
                 if (result == "Error 401") {
                           messageBox("error", __("Error in translation received status 401<br>The request is not authorized because credentials are missing or invalid."));
                            // alert("Error in translation received status 401 \r\nThe request is not authorized because credentials are missing or invalid.");
                 }
                      ...
                      messageBox("error", "Error in translation received status 429 :");
                     //alert("Error in translation received status 403 with readyState == 3 \r\nLanguage: " + language + " not supported!");
                     stop = true;
                 }
                 else {
            ...
            const result = await translateWithLara(
             ...
             if (result == "Error 401") {
                 ...
                 if (errorstate != "OK") {
                    ...
                    let editor = true;
                    result = await NLPCloudTranslate(plural, destlang, record, apikeyNLP, OpenAIPrompt, replacePreVerb, row, transtype, plural_line, formal, locale, convertToLower, editor, "1", MistralSelect, OpenAItemp, spellCheckIgnore, OpenAITone, "editor", openAiGloss);
                    if (result == "Error 401") {
                            messageBox("error", __("Error in translation received status 401<br>The request is not authorized because credentials are missing or invalid."));
                            // alert("Error in translation received status 401 \r\nThe request is not authorized because credentials are missing or invalid.");
                           stop = true
            ...
            if (result == "Error 401") {
                ...
                if (result == "Error 401") {
                    ...
                    if (errorstate != "OK") {
                        ...
                        let editor = false; 
                        result = await openRouterTranslate(plural, destlang, record, apikeyOpenRouter, OpenAIPrompt, replacePreVerb, row, transtype, plural_line, formal, locale, convertToLower, editor, "1", OpenRouterSelect, OpenAItemp, spellCheckIgnore, OpenAITone, "editor", openAiGloss);
                        if (result == "Error 401") {
                            messageBox("error", __("Error in translation received status 401<br>The request is not authorized because credentials are missing or invalid."));
                            // alert("Error in translation received status 401 \r\nThe request is not authorized because credentials are missing or invalid.");
                        }
...
async function translatePage(apikey, apikeyCerebras, apikeyDeepl, apikeyMicrosoft, apikeyKimi, apikeyOpenAI, apikeyMistral, apikeyClaude, apikeyDeepSeek, apikeyTranslateio, apikeyNLP, OpenAIPrompt, transsel, destlang, postTranslationReplace, preTranslationReplace, formal, convertToLower, DeeplFree, OpenAISelect, MistralSelect, openAIWait, OpenAItemp, spellCheckIgnore, deeplGlossary, OpenAITone, DeepLWait, openAiGloss, ClaudePrompt, ClaudeModel, apikeyOllama, LocalOllama, ollamaModel, ollamaPrompt, apikeyLingvanex, apikeyGemini, GeminiModel, GeminiPrompt, LMStudioWait, apikeyOpenRouter, OpenRouterSelect, apikeygroq, groqSelect, BatchSize, KimiSelect, CerebrasSelect, laraAccessKeyId, laraAccessKeySecret) {
    ...
    if (transsel == "groq") {
        ...
        //console.debug("we translate with Kimi")
        let is_editor = false
...
// CHANGES vs original:
// 1. Added `koboldUrl` parameter at the end of the function signature
            ...
            if (toTranslate) {
                ...
                if (pretrans == "notFound") {
                    ...
                    if (transsel == "LibreTrans") {
                        ...
                        let editor = true;
                        result = await KimiTranslate(original, destlang, e, apikeyKimi, OpenAIPrompt, replacePreVerb, rowId, transtype, plural_line, formal, locale, convertToLower, editor, "1", KimiSelect, OpenAItemp, spellCheckIgnore, OpenAITone, "editor", openAiGloss);
                        ...
                        let editor = true;
                        result = await MistralTranslate(original, destlang, e, apikeyMistral, OpenAIPrompt, replacePreVerb, rowId, transtype, plural_line, formal, locale, convertToLower, editor, "1", MistralSelect, OpenAItemp, spellCheckIgnore, OpenAITone, "editor", openAiGloss);
                        if (result == "Error 401") {
                            messageBox("error", __("Error in translation received status 401<br>The request is not authorized because credentials are missing or invalid."));
                        } else if (result == "Error 403") {
                            messageBox("error", "Error in translation received status 403 with readyState == 3<br>Language: " + destlang + " not supported!");
            ...
            if (checkplural != null) {
                ...
                if (pretrans == "notFound") {
                    ...
                    if (transsel == "LibreTrans") {
                        ...
                        let editor = true;
                        result = await MistralTranslate(original, destlang, e, apikeyMistral, OpenAIPrompt, replacePreVerb, rowId, transtype, plural_line, formal, locale, convertToLower, editor, "1", OpenAISelect, OpenAItemp, spellCheckIgnore, OpenAITone, "editor", openAiGloss);
                        if (result == "Error 401") {
                            messageBox("error", __("Error in translation received status 401<br>The request is not authorized because credentials are missing or invalid."));
                        } else if (result == "Error 403") {
                            messageBox("error", "Error in translation received status 403 with readyState == 3<br>Language: " + destlang + " not supported!");
...
function highlight(elem, keywords, caseSensitive = false, cls = 'highlight') {
    // Sanitize: keep only non-empty strings, remove duplicates
    const clean = [...new Set(keywords.filter(k => typeof k === 'string' && k.trim() !== ''))];
    ...
    // Escape special characters so keywords like "?" or "." are treated literally
    const keywordRegex = new RegExp(clean.map(escapeRegExp).join('|'), flags);

    Array.from(elem.childNodes).forEach(child => {
        if (child.nodeType !== 3) { // not a text node
            // Skip spans we created in an earlier pass, so repeated calls don't pile up
            if (child.classList && child.classList.contains(cls)) return;
            highlight(child, clean, caseSensitive, cls);
        } else if (keywordRegex.test(child.textContent)) {
            keywordRegex.lastIndex = 0; // reset after test() — 'g' regexes are stateful
            const text = child.textContent;
            const frag = document.createDocumentFragment();
            let lastIdx = 0;
            let match;
            while ((match = keywordRegex.exec(text)) !== null) {
                // Guard against zero-length matches — prevents infinite loop
                if (match[0].length === 0) { keywordRegex.lastIndex++; continue; }
...
async function onCopySuggestionClicked(target,rowId,replaceVerb) {
    chrome.storage.local.get(["postTranslationReplace", "convertToLower", "spellCheckIgnore", "ForceFormal"], async function (data) {
        ...
        mytextarea[0].addEventListener("click", (target) => {
            //if (detail_glossary) {
...
function checkBlankLabels() {
  ...
  rows.forEach(row => {
    ...
    const origStartsWithBlank = originalText.startsWith(" ");
    const origEndsWithBlank = originalText.endsWith(" ");
    const previewStartsWithBlank = previewText.startsWith(" ");
    const previewEndsWithBlank = previewText.endsWith(" ");

    const origEndsWithPeriod = originalText.endsWith(".");
    const previewEndsWithPeriod = previewText.endsWith(".");

    const origEndsWithQuestion = originalText.endsWith("?");
    const previewEndsWithQuestion = previewText.endsWith("?");

    const origEndsWithExclamation = originalText.endsWith("!");
    const previewEndsWithExclamation = previewText.endsWith("!");

    const labels = [];

    if (origStartsWithBlank && !previewStartsWithBlank) {
      labels.push("Missing start blank in preview");
    }
    if (!origStartsWithBlank && previewStartsWithBlank) {
      labels.push("Additional blank at start");
    }
    if (origEndsWithBlank && !previewEndsWithBlank) {
      labels.push("Missing blank at the end");
    }
    if (!origEndsWithBlank && previewEndsWithBlank) {
      labels.push("Additional blank at the end");
    }
    if (origEndsWithPeriod && !previewEndsWithPeriod) {
      labels.push("Missing period in preview");
    }
    if (!origEndsWithPeriod && previewEndsWithPeriod) {
      labels.push("Additional period in preview");
    }
    if (origEndsWithQuestion && !previewEndsWithQuestion) {
      labels.push("Missing question mark in preview");
    }
    if (!origEndsWithQuestion && previewEndsWithQuestion) {
      labels.push("Additional question mark in preview");
    }
    if (origEndsWithExclamation && !previewEndsWithExclamation) {
      labels.push("Missing exclamation mark in preview");
    }
    if (!origEndsWithExclamation && previewEndsWithExclamation) {
